...
def get_optimizer(
    params, optimizer_type="adam", learning_rate=1e-6, optimizer_params=None
):
    ...
    if lower_type.startswith("dadaptation"):
        ...
        if lower_type.endswith("lion"):
            optimizer = dadaptation.DAdaptLion(
                params, eps=1e-6, lr=use_lr, **optimizer_params
            )
            ...
            print(
                "WARNING: Dadaptation optimizer type has been changed to DadaptationAdam. Please update your config."
            )
        ...
        optimizer = Adam8bit(
            params, lr=learning_rate, eps=1e-6, decouple=True, **optimizer_params
        )
        ...
        if lower_type == "adam8bit":
            return bitsandbytes.optim.Adam8bit(
                params, lr=learning_rate, eps=1e-6, **optimizer_params
            )
        if lower_type == "ademamix8bit":
            return bitsandbytes.optim.AdEMAMix8bit(
                params, lr=learning_rate, eps=1e-6, **optimizer_params
            )
        ...
        try:
            ...
            raise ImportError(
                "Please install lion_pytorch to use Lion optimizer -> pip install lion-pytorch"
            )
